...
template<typename T> const T Node<T>::getElement(){return this->e;}
template<typename T> const Node<T>* Node<T>::getNext(){return this->next;}
...
template<typename T> const string Node<T>::toString(){
    ostringstream oss;
    oss << "{["<< &this->element << "|" << this->element << "]-" << this->next << "->}";
    return oss.str();
}

template<typename T> const bool Node<T>::equalsTo(const void* other){
