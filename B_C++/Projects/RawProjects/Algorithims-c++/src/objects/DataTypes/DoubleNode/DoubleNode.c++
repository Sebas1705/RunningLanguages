...
template<typename T> T DoubleNode<T>::getElement(){return this->e;}
template<typename T> DoubleNode<T>* DoubleNode<T>::getNext(){return this->next;}
template<typename T> DoubleNode<T>* DoubleNode<T>::getPrev(){return this->prev;}
...
template<typename T> string DoubleNode<T>::toString(){
    ostringstream oss;
    oss << "{<-" << this->prev <<"-["<< &this->element << "|" << this->element << "]-" << this->next << "->}";
    return oss.str();
}

template<typename T> bool DoubleNode<T>::equalsTo(const void* other){
